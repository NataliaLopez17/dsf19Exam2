...
//						temp = temp.getNext());
	...
	/*
	 * Write a non-member method majorityElement which receives a list of integers L. The method returns the majority element, which is the element that appears the most in the list L.  You might assume that there is only one majority element in the list L, and that L is not empty. For example if L = {10, 12, 1, 10, 10, 2, -1, 0, 2}, then the method returns 10.

Hint: use a map to track the number frequency. Also, recall that keys and values are returned by the map methods in a corresponding order (i.e., the first key in the list of keys corresponds to the first value in the list of values).
	 */
